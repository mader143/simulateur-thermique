...
class InterfaceProto:
    ...
    def lancer_dino(self):
        # Chemin absolu vers main.py basé sur le dossier du script interface.py
        script_dir = os.path.dirname(os.path.abspath(__file__))
        game_path = os.path.join(script_dir, "main.py")

        if not os.path.exists(game_path):
            messagebox.showerror("Erreur", f"Le fichier main.py est introuvable à l'emplacement :\n{game_path}")
            return

        try:
            subprocess.Popen([sys.executable, game_path])
            print("Jeu du dino lancé.")
        except Exception as e:
