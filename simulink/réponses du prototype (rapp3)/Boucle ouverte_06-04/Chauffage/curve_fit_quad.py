...
formule = (f"P = {a:+.4f}·DC²  {signe(b)} {abs(b):.4f}·T²  {signe(c)} {abs(c):.4f}·DC·T\n"
           f"    {signe(d)} {abs(d):.4f}·DC  {signe(e)} {abs(e):.4f}·T  {signe(f)} {abs(f):.4f}")

print(f"R² = {r2:.4f}")
print(f"\nFormule:\n{formule}")

# --- Grille pour la surface (s'adapte aux données) ---
...
# Lignes verticales des points vers la surface (pour visualiser l'erreur)
for _, row in data.iterrows():
    p_fit = model_quadratique(np.array([[row['duty'], row['temp']]]), *popt)[0]
    ax.plot([row['duty'], row['duty']], [row['temp'], row['temp']],
            [row['puissance'], p_fit], color='red', linewidth=0.8, alpha=0.6)
...
# Formule + R² sur le graphique
texte = f"$R^2 = {r2:.4f}$\n\n" \
        f"$P = {a:.4f}\\cdot DC^2 {signe(b)} {abs(b):.4f}\\cdot T^2 {signe(c)} {abs(c):.4f}\\cdot DC\\cdot T$\n" \
        f"$\\quad {signe(d)} {abs(d):.4f}\\cdot DC {signe(e)} {abs(e):.4f}\\cdot T {signe(f)} {abs(f):.4f}$"

fig.text(0.01, 0.02, texte, fontsize=10, verticalalignment='bottom',
