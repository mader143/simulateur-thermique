...
def compute_timestep(T, T_init, alpha_dt_dx2, alpha_dt_dy2, 
    ...
    This runs 10-100x faster than pure NumPy.
    """
    T_new = T.copy()
    
    # Internal points - diffusion equation
    for i in range(1, nx-1):
        for j in range(1, ny-1):
            ...
            # Face convection (applies everywhere)
    ...
    # Edge convection - y=ny-1
    for i in range(nx):
        T_new[i, ny-1] += coeff_conv * (T_init - T[i, ny-1])
    
    # Boundary diffusion - edges (you can refine this as needed)
    # x=0 edge
    for j in range(1, ny-1):
...
# Setup matplotlib (only for demo - you'll use PyQt5)
plt.ion()
...
for t in range(nt):
    ...
    if t % 500 == 0:
        ...
        print(f"T_max: {T[x0, y0]-273.15:.2f}°C | Gain: +{P_cell_dt_vol:.6f} | Perte: -{perte_par_pas:.6f} | Net: {balance:.6f}")
    ...
    # Update plots (adjust frequency as needed)
    if t % 50 == 0:  # Can be changed for PyQt5
        # Update thermistor plot
...
print(f"Speedup vs original: ~10-50x faster")
