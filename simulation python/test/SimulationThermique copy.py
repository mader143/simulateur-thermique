...
class SimulationThermique(QObject):
    ...
    def simuler_diffusion(self):
        ...
        therm1_locx = int(14.87e-3 / self.dx)
        therm1_locy = int((self.largeur / 2) / self.dx)
        therm2_locx = int(59.35e-3 / self.dx)
        therm2_locy = int((self.largeur / 2) / self.dx)
        therm3_locx = int(104.99e-3 / self.dx)
        therm3_locy = int((self.largeur / 2) / self.dx)
        ...
        for t in range(nt):
            ...
            # Émettre les données — SimControl s'occupe du rendu dans le thread principal
            if t % PLOT_INTERVAL == 0 and (current_time - last_2d_update) > 0.05:
                self.therm_1.emit('Thermistance', temps, T1, T2, T3)
                last_2d_update = current_time

            if t % PLOT_INTERVAL == 0 and (current_time - last_3d_update) > 0.1:
            ...
            if t % 500 == 0:
                ...
                if t > 0:
                    ...
                    print(f"Progress: {progress:.1f}% | Elapsed: {elapsed:.1f}s | ETA: {est_remaining:.1f}s | Speed: {speed:.1f}x", end='\r')
                else:
